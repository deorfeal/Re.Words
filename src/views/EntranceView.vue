...
function validateForm(): boolean {
    ...
    if (form.value.password.value.length < 6) {
        ...
        alertStore.showAlert('Password must be at least 6 characters', 'alert--error');
        isValid = false;
    } else {
    ...
    if (!isLogin.value && form.value.password.value !== form.value.passwordRepeat.value) {
...
const singUp = async (): Promise<void> => {
    ...
    try {
        await createUserWithEmailAndPassword(getAuth(), form.value.email.value, form.value.password.value)
        router.push('/menu')
    } catch (error: unknown) {
...
const singIn = async (): Promise<void> => {
    loaderStore.showLoader()
    try {
        await signInWithEmailAndPassword(getAuth(), form.value.email.value, form.value.password.value)
        router.push('/menu')
    } catch (error: unknown) {
        if (error instanceof Error) {
            alertStore.showAlert(`${error.message}`, 'alert--error');
            ...
            <div class="entrance__inner">
                ...
                <form class="entrance__form form" action="#" method="post">
                    <div class="form__box form-box">
                        <input class="form-box__input filling filling--input"
                            :class="{ 'filling--error': form.email.error }" type="email" placeholder="Email"
                            v-model="form.email.value">
                    </div>
                    <div class="form__box form-box">
                        <input class="form-box__input filling filling--input"
                            :class="{ 'filling--error': form.password.error }" :type="passwordType"
                            placeholder="Password" v-model="form.password.value">
                        <div class="form-box__show" @click="togglePasswordShow">
                            <TransitionGroup name="opacity">
                                <svg v-if="!passwordIsShow" width="16" height="16" viewBox="0 0 16 16" fill="none"
                    ...
                    <div v-if="!isLogin" class="form__box form-box">
                        <input class="form-box__input filling filling--input"
                            :class="{ 'filling--error': form.passwordRepeat.error }" :type="passwordType"
                            placeholder="Repeat password" v-model="form.passwordRepeat.value">
                        <div class="form-box__show" @click="togglePasswordShow">
                            <TransitionGroup name="opacity">
                                <svg v-if="!passwordIsShow" width="16" height="16" viewBox="0 0 16 16" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
